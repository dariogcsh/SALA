...
class notificacionGestionFinde extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'notificacion:gestionfinde';
    ...
    protected $description = 'Este comando notificara a la sucursal correspondiente que hay una asistencia de 
                            máquina parada aún en gestion, cada 60 minutos los fines de semana';
    ...
    public function handle(NotificationsService $notificationsService)
    {
        ...
        if (isset($asistencias)){
            foreach ($asistencias as $asist) {
                ...
                $horaLimite = $horaAsist->modify('+ 120 minutes');
                if ($horaActual >= $horaLimite){
                ...
                $horaLimite = $horaAsist->modify('+ 180 minutes');
                if ($horaActual >= $horaLimite){
                ...
                $horaLimite = $horaAsist->modify('+ 240 minutes');
                if ($horaActual >= $horaLimite){
                ...
                $horaLimite = $horaAsist->modify('+ 300 minutes');
                if ($horaActual >= $horaLimite){
