...
class Varios
{
		...
		public function registros_sin_cultivo(){
			include("conexion.php");
			$query = "SELECT NumSMaq, FecIUtil, HorIUtil, FecFUtil, HorFUtil FROM utilidad WHERE CultUtil = '0'  ORDER BY CultUtil DESC LIMIT 52";
			$result = $conn->query($query);
			$conn->close();
			return $result;
		}
		...
		public function buscar_cultivo($finicio, $hinicio, $ffin, $hfin, $maquina){
			include("conexion.php");
			$query = "SELECT CateUtil FROM utilidad WHERE FecIUtil = '". $finicio ."' AND HorIUtil = '". $hinicio ."' AND FecFUtil = '". $ffin ."' AND HorFUtil = '". $hfin ."' AND NumSMaq = '". $maquina ."' AND (CateUtil LIKE '%Tiempo en Soja%' OR CateUtil LIKE '%Tiempo en maíz%' OR CateUtil LIKE '%Tiempo en frijones o guisantes%' OR CateUtil LIKE '%Tiempo en maíz para palomitas%')";
			$result = $conn->query($query);
			$conn->close();
