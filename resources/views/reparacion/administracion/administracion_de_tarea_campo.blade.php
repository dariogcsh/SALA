        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span class="align-baseline" style="font-size: 1rem;">{{ __('Administracion de tareas de campo') }}</span>
                    <a id="download" class="btn btn-secondary" href={{ route('reparacion.tarea_campo_pdf',$reparacion->id) }}>Descargar PDF</a>
                </div>
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-6">
                            <input type="time" class="form-control" value="{{ $reparacion->horario_salida_agencia }}" readonly> 
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label text-md-right">{{ __('Horario llegada campo') }}</label>
                        <div class="col-md-6">
                            <input type="time" class="form-control" value="{{ $reparacion->horario_llegada_campo }}" readonly> 
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label text-md-right">{{ __('Horario salida campo') }}</label>
                        <div class="col-md-6">
                            <input type="time" class="form-control" value="{{ $reparacion->horario_salida_campo }}" readonly> 
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-4 col-form-label text-md-right">{{ __('Horario llegada agencia') }}</label>
                        <div class="col-md-6">
                            <input type="time" class="form-control" value="{{ $reparacion->horario_llegada_agencia }}" readonly> 
                        </div>
                    </div>
                    ...
                    @else
                        ...
                        <br>
                    @endif
                    @if(!$observaciones_reparacion->isEmpty())
                        <b>Reclamos</b>
                        <br>
                        @foreach ($observaciones_reparacion as $i => $observacion)
                            <b>Reclamo {{ $i + 1}}</b>
                            <div class="form-group row">
                                <label class="col-md-4 col-form-label text-md-right">{{ __('Fecha') }}</label>
                                <div class="col-md-6"> 
                                    <input type="date" class="form-control" value="{{ $observacion->fecha }}" readonly>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label class="col-md-4 col-form-label text-md-right">{{ __('Descripcion') }}</label>
                                <div class="col-md-6"> 
                                    <textarea class="form-control-textarea" rows="4" readonly>{{ $observacion->descripcion }}</textarea> 
                                </div>
                            </div>
                        @endforeach
                        <br>
                    @else
                        <b>No hay reclamos relacionados</b>
                        <br>
                    @endif
                    <div class="form-group row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-md-4" style="margin-bottom:5px;">
                            <a class="btn btn-warning btn-block" href={{ route('reparacion.administrar_tarea_de_campo_editar',$reparacion->id) }}>Editar</a>
                        </div>
                        <div class="col-xs-12 col-md-4" style="margin-bottom:5px;">
                                <form method="post" action="{{ route('reparacion.borrar_tarea_campo',$reparacion->id) }}">
                                    @csrf
                                    @method('delete')
...
<script>
    ...
    document.getElementById("download").addEventListener("click", function(){
        ...
        const type = "tareas/campo";
        const name = `tarea_campo${id}`
