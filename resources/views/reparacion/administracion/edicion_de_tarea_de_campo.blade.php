...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Edicion de tarea de campo') }}</div>
                <div class="card-body">
                    @include('custom.message')
                    <form method="POST" action="{{ route('reparacion.editar_tarea_campo') }}">
                        @method('PUT')
                        @csrf
                            ...
                            <div class="col-md-6">
                                <select class="form-control" name="id_sucursal" id="id_sucursal" required autofocus> 
                                    ...
                                    @foreach ($sucursals as $sucursal)
                                        <option value="{{ $sucursal->id }}" {{ ($reparacion->id_sucursal == $sucursal->id) ? "selected" : ""  }}>{{ $sucursal->NombSucu }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input type="time" class="form-control" name="horario_salida_agencia" id="horario_salida_agencia" value="{{ $reparacion->horario_salida_agencia }}" required autofocus> 
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label text-md-right">{{ __('Horario llegada campo') }} *</label>
                            <div class="col-md-6">
                                <input type="time" class="form-control" name="horario_llegada_campo" id="horario_llegada_campo" value="{{ $reparacion->horario_llegada_campo }}" required autofocus> 
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label text-md-right">{{ __('Horario salida campo') }} *</label>
                            <div class="col-md-6">
                                <input type="time" class="form-control" name="horario_salida_campo" id="horario_salida_campo" value="{{ $reparacion->horario_salida_campo }}" required autofocus> 
                            </div>
                        </div>
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label text-md-right">{{ __('Horario llegada agencia') }} *</label>
                            <div class="col-md-6">
                                <input type="time" class="form-control" name="horario_llegada_agencia" id="horario_llegada_agencia" value="{{ $reparacion->horario_llegada_agencia }}" required autofocus> 
                            </div>
                        </div>                        
                        <b>Tareas</b>
                        <br>
                        ...
                        <b>Reclamos</b>
                        <br>
                        <div class="reclamos_guardadas">
                            @foreach ($observaciones_reparacion as $i => $reclamo)
                                <div>
                                    <b>Reclamo {{ $i + 1}}</b>
                                    <div class="form-group row">
                                        <label class="col-md-4 col-form-label text-md-right">{{ __('Fecha') }} *</label>
                                        <div class="col-md-6"> 
                                            <input type="date" class="form-control" name="{{ 'fecha_reclamo'. ($reclamo->id) }}" id="{{ 'fecha_reclamo'. ($reclamo->id) }}" value="{{ $reclamo->fecha }}" required autofocus>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label class="col-md-4 col-form-label text-md-right">{{ __('Descripcion') }} *</label>
                                        <div class="col-md-6"> 
                                            <textarea class="form-control-textarea" rows="4" name="{{ 'descripcion_reclamo'. ($reclamo->id) }}" id="{{ 'descripcion_reclamo'. ($reclamo->id) }}" required autofocus>{{ $reclamo->descripcion }}</textarea> 
                                        </div>
                                    </div>
                                    <div class="form-group row mb-0">
                                        <div class="col-md-6 offset-md-4"> 
                                            <a href="#" class="borrar_reclamo_guardado btn btn-danger" value="{{ $reclamo->id }}">
                                                Borrar
                                            </a> 
                                        </div>
                                    </div>
                        ...
                        <input type="hidden" name="cant_reclamos" id="cant_reclamos" value="{{ count($observaciones_reparacion) }}">
                        <input type="hidden" name="cant_reclamos_a_crear" id="cant_reclamos_a_crear" value="0">
                        <input type="hidden" name="cant_reclamos_a_editar" id="cant_reclamos_a_editar" value="{{ count($tareas_reparacion) }}">
                        <input type="hidden" name="cant_reclamos_a_eliminar" id="cant_reclamos_a_eliminar" value="0">
                        <input type="hidden" name="ids_reclamos_a_eliminar" id="ids_reclamos_a_eliminar" value="">
                        @php
                            $ids_reclamos_a_editar = [];
                            foreach ($observaciones_reparacion as $i => $reclamo) {
                                $ids_reclamos_a_editar[$i - 1] = $reclamo->id; 
                            } 
                            $ids_reclamos_a_editar = implode($ids_reclamos_a_editar, ",") . ",";
                        @endphp
                        <input type="hidden" name="ids_reclamos_a_editar" id="ids_reclamos_a_editar" value="{{ $ids_reclamos_a_editar }}">
                        <input type="hidden" name="id" value="{{ $reparacion->id }}">
                        <br>
                        <div class="form-group row">
                            <label class="col-md-4 col-form-label text-md-right">{{ __('Firma tecnico') }} *</label>
                            <div class="col-md-6">
...
<script type="application/javascript">
    $(document).ready(function() {
        ...
        var reclamos_guardadas = $('.reclamos_guardadas');
        var cant_reclamos_input = document.getElementById('cant_reclamos');
        var borrar_reclamo_guardado = document.getElementById('borrar_reclamo_guardado');
        var cant_reclamos_a_crear = document.getElementById('cant_reclamos_a_crear');        
        var cant_reclamos_a_editar = document.getElementById('cant_reclamos_a_editar');        
        var cant_reclamos_a_eliminar = document.getElementById('cant_reclamos_a_eliminar');        
        var ids_reclamos_a_eliminar = document.getElementById('ids_reclamos_a_eliminar');
        var ids_reclamos_a_editar = document.getElementById('ids_reclamos_a_editar'); 

        var cant_reclamos = cant_reclamos_input.value;
        $(agregar_reclamo).click(function(e) {
            e.preventDefault();
            if(cant_reclamos_a_crear.value > 0){
                let borrar_reclamo = document.getElementById('borrar_reclamo');
                borrar_reclamo.remove();
            }
            cant_reclamos++;
            cant_reclamos_input.value = cant_reclamos;
            cant_reclamos_a_crear.value++;
            $(reclamos).append(`<div class="mt-2" id="reclamo_${cant_reclamos}"> <b>Reclamo ${cant_reclamos}</b> <div class="form-group row"><label class="col-md-4 col-form-label text-md-right">{{ __('Fecha') }} *</label> <div class="col-md-6"> <input type="date" class="form-control" name="fech_reclamo${cant_reclamos_a_crear.value}" id="fech_reclamo${cant_reclamos_a_crear.value}"> </div> </div> <div class="form-group row"><label class="col-md-4 col-form-label text-md-right">{{ __('Descripcion') }} *</label> <div class="col-md-6"> <textarea class="form-control-textarea" name="descr_reclamo${cant_reclamos_a_crear.value}" id="descr_reclamo${cant_reclamos_a_crear.value}" rows="4"></textarea> </div> </div> <div class="form-group row mb-0"> <div class="col-md-6 offset-md-4" id="borrar_reclamo"> <a href="#" class="borrar_reclamo btn btn-danger">Borrar</a> </div> </div></div>`);
        });
        ...
        $(reclamos_guardadas).on("click", ".borrar_reclamo_guardado", function(e) {
            e.preventDefault();
            cant_reclamos_a_editar.value = cant_reclamos_a_editar.value - 1;
            cant_reclamos--;
            cant_reclamos_input.value = cant_reclamos;
            cant_reclamos_a_eliminar.value++;
            var id = e.target.getAttribute('value');
            ids_reclamos_a_eliminar.value = ids_reclamos_a_eliminar.value + id + ",";
            ids_reclamos_a_editar.value = ids_reclamos_a_editar.value.replace(id + ",", "");
            if(ids_reclamos_a_editar.value[0] && ids_reclamos_a_editar.value.charAt(0) == ","){
                ids_reclamos_a_editar.value = ids_reclamos_a_editar.value.substring(1);
            }
            $(this).parent('div').parent('div').parent('div').remove();
        });
